...
import {Membership} from '../Base/index'
...
class Dashboard extends Component {
  ...
  render() {
    ...
    return (
      <div className="animated fadeIn">
        ...
        <Row>
          <Col xs="12" sm="6" lg="6">
            {this.state.userData !== null? this.state.userData.map(x => (
              <Membership 
                key={x.id} 
                id={x.id} 
                clubName={x.club.clubName} 
                sportArt={x.club.mainSportForm} 
                userRole={x.userRole} 
                sinceDate={x.club.createdAt}
                buttonAction={()=> this.removeMembership(x.id)}
              />
            )) : null}
          </Col>
        </Row>
  ...
  userData(){
    ...
    .then(userData => this.setState({ userData : userData['memberships'] }) );
  }

  removeMembership(id){
      let userData = this.state.userData;
      let toRemove = userData.map(x=> x.id).indexOf(id);
      
      userData.splice(toRemove, 1);
